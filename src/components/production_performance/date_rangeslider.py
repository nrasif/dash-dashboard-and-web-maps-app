...
#     def sync_date_range_slider(date_rangeslider, from_date, to_date) -> list[str]:
#     # If date range slider is changed
#         dataframe = source.filter(from_date=from_date, to_date=to_date).to_dataframe
        
#         if date_rangeslider is not None and from_date is not None and to_date is not None:
...
#             if selected_from_date != dataframe[ProductionDataSchema.DATE][start_index] or selected_to_date != dataframe[ProductionDataSchema.DATE][end_index]:
...
#         elif date_rangeslider is None and from_date is not None and to_date is not None:
#             selected_from_date = pd.to_datetime(from_date)
#             selected_to_date = pd.to_datetime(to_date)
...
#                 marks={i: date.strftime('%Y-%m-%d') for i, date in enumerate(source.to_dataframe[ProductionDataSchema.DATE])},
#                 min=0,
#                 max=len(source.to_dataframe) - 1,
#                 value=[0, len(source.to_dataframe) - 1]
#             ),      
#         ]
#     )